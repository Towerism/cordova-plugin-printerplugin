...
var CordovaPrint = {
  ...
  barcode: function(
    ...
    exec(fnSuccess, fnError, "PrintingPlugin", "barcode", [
      barStr,
      barType,
      barWidth,
      barHeight,
  ...
  qr_Code: function(fnSuccess, fnError, str, model, size, eclevel) {
    ...
    exec(fnSuccess, fnError, "PrintingPlugin", "qR_Code", [
      justify_center +
      "\x1D\x28\x6B\x04\x00\x31\x41" +
      qr_model +
      "\x00" + // Select the model
      "\x1D\x28\x6B\x03\x00\x31\x43" +
      qr_size + // Size of the model
      "\x1D\x28\x6B\x03\x00\x31\x45" +
      qr_eclevel + // Set n for error correction
      "\x1D\x28\x6B" +
      qr_pL +
      qr_pH +
      "\x31\x50\x30" +
  ...
  SETSIZE: function(width, height) {
    var command =
      "1D21" + ("00" + (width - 1) * 16 + (height - 1).toString(16)).slice(-2);
    return command;
  },
  SMOOTH_ON: "1D62FF",
  SMOOTH_OFF: "1D6200",
...
function validateBarcode(ESCPOS_BARTYPE, ESCPOS_BARCONTENT) {
  switch (ESCPOS_BARTYPE) {
    ...
    case 65:
      ...
      if (!/^[0-9]+$/.test(ESCPOS_BARCONTENT)) return false;
      break;
    case "UPC_B":
    case 66:
      if (
        !(ESCPOS_BARCONTENT.length >= 6 && ESCPOS_BARCONTENT.length <= 8) &&
        ESCPOS_BARCONTENT.length != 11 &&
        ESCPOS_BARCONTENT.length != 12
      )
        return false;
    ...
    case 69:
      ...
      // 0 – 9, A – Z, SP, $, %, *, +, -, ., /
      if (!/^[0-9A-Z $%*+-./]+$/.test(ESCPOS_BARCONTENT)) return false;

      break;
    case "Interleaved_2of5":
    case 70:
      if (
        ESCPOS_BARCONTENT.length % 2 !== 0 ||
        !(ESCPOS_BARCONTENT.length >= 2 && ESCPOS_BARCONTENT.length <= 254)
      )
        return false;
      // Just allow numeric values
      if (!/^[0-9]+$/.test(ESCPOS_BARCONTENT)) return false;
    ...
    case 71:
      ...
      // 0 – 9, A – D, a – d, $, +, −, ., /, :
      if (!/^[0-9A-Da-d $+-./:]+$/.test(ESCPOS_BARCONTENT)) return false;
      break;
    case "Code_93":
    case 72:
      if (!(ESCPOS_BARCONTENT.length >= 2 && ESCPOS_BARCONTENT.length <= 255))
      ...
      // all ascii
      if (!/^[\x00-\x7F]+$/.test(ESCPOS_BARCONTENT)) return false;
    ...
    case 73:
      ...
      if (!/^({A|{B|{C)[\x00-\x7F]+$/.test(ESCPOS_BARCONTENT)) return false;
      break;
    case "UCC_Ean_128":
    case 74:
      if (!(ESCPOS_BARCONTENT.length >= 2 && ESCPOS_BARCONTENT.length <= 255))
